...
# This causes convergence issues with the JAGS model but shouldn't affect
# Overall performance
# 1. Using lme4
# 2. Using JAGS
...
model {
  ...
  for (j in 1:len_group1) {
    tree1[j] ~ dnorm(mu1, sigma1^-2)
  }
  for (j in 1:len_group2) {
    tree2[j] ~ dnorm(mu2, sigma2^-2)
  }
  
  # These should probably be set better to see if the convergence improves
  mu1 ~ dnorm(0, 10^-2) 
  mu2 ~ dnorm(0, 10^-2)
...
# tree2[k] ~ N(mu2, sigma2^2)
# mu1, mu2 ~ N(0, a^2), a = 10
...
for (i in 1:num_iter) {
  ...
  R1 <- y - M2 %*% tree2 # Partial residuals for tree 1
  ...
  R2 <- y - M1 %*% tree1 # Partial residuals for tree 2
  ...
  tree2 <- t(mvnfast::rmvn(1, solve(prec, tau * crossprod(M2, R2)), solve(prec)))
